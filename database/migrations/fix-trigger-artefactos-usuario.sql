-- ============================================================================
-- FIX: Trigger de artefactos - corregido para usar la estructura correcta
-- La tabla auditoria_artefactos_domesticos tiene: id_auditoria_solicitante, artefacto, estado
-- ============================================================================
...
BEGIN
    ...
    -- Obtener el usuario que actualiza (buscar en múltiples variables de sesión)
    BEGIN
    ...
    -- Si aún no hay usuario, NO auditar (evitar errores)
    IF v_usuario IS NULL OR v_usuario = '' THEN
        IF TG_OP = 'INSERT' THEN
            RETURN NEW;
        ELSE
    ...
    -- Si el solicitante no existe (está siendo eliminado), no auditar cambios de artefactos
    ...
    -- Insertar registro de auditoría (todos los campos del solicitante iguales, ya que solo cambian artefactos)
    ...
    -- Registrar el artefacto cambiado usando la estructura CORRECTA de la tabla
    IF TG_OP = 'INSERT' THEN
