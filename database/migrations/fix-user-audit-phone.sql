-- Agregar columnas de teléfono a la tabla de auditoría si no existen
ALTER TABLE auditoria_actualizacion_usuarios ADD COLUMN IF NOT EXISTS telefono_celular_anterior VARCHAR(20);
ALTER TABLE auditoria_actualizacion_usuarios ADD COLUMN IF NOT EXISTS telefono_celular_nuevo VARCHAR(20);

-- Actualizar el procedimiento almacenado para incluir la auditoría del teléfono
...
AS $$
    ...
    BEGIN
        ...
        IF (v_nombres_anterior IS DISTINCT FROM COALESCE(p_nombres, v_nombres_anterior)) OR
           ...
           (v_habilitado_sistema_anterior IS DISTINCT FROM v_habilitado_sistema_anterior) OR -- Este no cambia aqui pero por consistencia
