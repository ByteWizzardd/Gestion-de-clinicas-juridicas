-- ============================================================================
-- FIX: Reinstalar trigger de actualización de solicitantes
-- Asegurar que detecta cambios en TODOS los campos, incluido sexo
-- ============================================================================
...
BEGIN
    ...
    IF v_usuario IS NULL OR v_usuario = '' THEN
        -- Si no hay usuario, no auditar (evitar usar 'SISTEMA')
        RETURN NEW;
    END IF;
    
    -- Verificar si hay CUALQUIER cambio en la tabla solicitantes
    -- Usando COALESCE para manejar NULLs correctamente
    ...
    IF v_hay_cambio THEN
        ...
        -- Obtener datos de familia (DESPUÉS) - en trigger AFTER, ya están actualizados
        SELECT 
            fh.jefe_hogar,
            ne.descripcion,
            fh.ingresos_mensuales
