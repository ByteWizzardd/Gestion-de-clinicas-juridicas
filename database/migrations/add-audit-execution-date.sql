-- Migración: Agregar auditoría para fecha de ejecución en acciones
-- Fecha: 2026-01-17

-- 1. Agregar columnas para almacenar cambios en fecha de ejecución
ALTER TABLE auditoria_actualizacion_acciones
ADD COLUMN IF NOT EXISTS fecha_ejecucion_anterior TEXT,
ADD COLUMN IF NOT EXISTS fecha_ejecucion_nuevo TEXT;

-- 2. Actualizar el trigger para capturar cambios en ejecutores y fechas desde variables de sesión
...
DECLARE
    ...
    v_fecha_ejecucion_anterior TEXT;
    v_fecha_ejecucion_nuevo TEXT;
BEGIN
    BEGIN
        ...
        v_fecha_ejecucion_anterior := current_setting('app.fecha_ejecucion_anterior', true);
        v_fecha_ejecucion_nuevo := current_setting('app.fecha_ejecucion_nuevo', true);
    EXCEPTION
        WHEN OTHERS THEN
            v_usuario := NULL;
            v_ejecutores_anterior := NULL;
            v_ejecutores_nuevo := NULL;
            v_fecha_ejecucion_anterior := NULL;
            v_fecha_ejecucion_nuevo := NULL;
    END;
    ...
    IF v_fecha_ejecucion_anterior = '' THEN v_fecha_ejecucion_anterior := NULL; END IF;
    IF v_fecha_ejecucion_nuevo = '' THEN v_fecha_ejecucion_nuevo := NULL; END IF;
    
    -- Detectar cambios en campos nativos O en ejecutores/fechas (pasados por sesión)
    IF (OLD.detalle_accion IS DISTINCT FROM NEW.detalle_accion OR
        ...
        v_ejecutores_anterior IS DISTINCT FROM v_ejecutores_nuevo OR
        v_fecha_ejecucion_anterior IS DISTINCT FROM v_fecha_ejecucion_nuevo) THEN
        ...
        INSERT INTO auditoria_actualizacion_acciones (
            ...
            fecha_ejecucion_anterior, fecha_ejecucion_nuevo,
            ...
            v_fecha_ejecucion_anterior, v_fecha_ejecucion_nuevo,
            v_usuario
        );
    END IF;
