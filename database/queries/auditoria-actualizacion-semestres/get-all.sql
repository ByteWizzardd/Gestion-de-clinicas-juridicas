-- Obtener todas las actualizaciones de semestres con filtros opcionales
SELECT 
    a.id,
    a.term,
    a.term_anterior,
    ...
    to_char(a.fecha_fin_nuevo, 'YYYY-MM-DD') as fecha_fin_nuevo,
...
FROM auditoria_actualizacion_semestres a
...
WHERE 
    ...
    AND (
        $4::TEXT IS NULL 
        OR a.term::TEXT ILIKE '%' || $4 || '%'
        OR a.fecha_inicio_anterior::TEXT ILIKE '%' || $4 || '%'
        OR a.fecha_inicio_nuevo::TEXT ILIKE '%' || $4 || '%'
        OR a.fecha_fin_anterior::TEXT ILIKE '%' || $4 || '%'
        OR a.fecha_fin_nuevo::TEXT ILIKE '%' || $4 || '%'
    )
ORDER BY 
