...
export default function AuditTypeCard({
    title,
    description,
    count,
    icon: Icon,
    ...
    return (
        <div className="bg-[var(--card-bg)] border-[var(--card-border)] border rounded-2xl md:rounded-3xl shadow-[0px_4px_10px_0px_rgba(0,0,0,0.30)] relative overflow-hidden px-4 sm:px-6 md:px-8 py-4 md:py-5 min-h-[160px] md:min-h-[180px] flex flex-col w-full transition-colors">
            ...
            <h3 className="text-base sm:text-lg md:text-xl font-medium text-[var(--card-text)] mb-1.5 relative z-10 transition-colors">
                {title}
            </h3>

            {/* Descripción */}
            <p className="text-xs md:text-sm text-[var(--card-text-muted)] mb-3 relative z-10 grow leading-relaxed transition-colors">
                {description}
            </p>

            {/* Contador y botón */}
            <div className="flex items-center justify-between gap-2 relative z-10">
                <div className="text-xl sm:text-2xl md:text-3xl font-semibold text-[var(--card-text)] transition-colors">
                    {count}
                </div>
                <Link href={href}>
