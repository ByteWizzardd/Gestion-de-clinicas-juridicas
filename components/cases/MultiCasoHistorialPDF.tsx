...
export function MultiCasoHistorialPDF({ data, logoBase64 }: MultiCasoHistorialPDFProps) {
    return (
        <Document>
            {data.map((casoItem, index) => {
                const { caso, acciones = [], citas = [], soportes = [], beneficiarios = [], equipo = [], cambiosEstatus = [] } = casoItem;
                const casoData = caso || {};
                ...
                return (
                    <Page key={index} size="A4" style={styles.page}>
                        ...
                        <View style={[styles.largeTextBox, { minHeight: 60 }]}>
                            <Text style={styles.valueText}>
                                {acciones.map((a: any) => `${formatDate(a.fecha_registro)}: ${a.detalle_accion}`).join('. ')}
                            </Text>
                        </View>
                        ...
                        <View style={{ marginLeft: 10 }}>
                            ...
                            {cambiosEstatus.map((c: any, i: number) => (
