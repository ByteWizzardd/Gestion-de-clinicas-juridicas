...
interface EditableAvatarProps {
  ...
  allowUpload?: boolean; // Si es false, solo permite eliminar, no subir
}

export default function EditableAvatar({ fotoPerfil, cedula, nombreInicial, onPhotoUpdated, onSuccess, allowUpload = true }: EditableAvatarProps) {
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const result = await uploadFotoPerfilUsuarioAction(formData);
  ...
  const handleDelete = async () => {
    ...
    try {
      const result = await deleteFotoPerfilUsuarioAction(cedula);
  ...
  // Determinar si el avatar es interactivo (solo si hay foto o se permite subir)
  const isInteractive = fotoPerfil || allowUpload;
      ...
      >
        ...
        <AnimatePresence>
          {showMenu && (
            ...
            {fotoPerfil && (
              <>
                {allowUpload && <div className="border-t border-gray-100 my-1"></div>}
                <button
