...
/**
 * Queries para la entidad AuditoriaActualizacionNivelesEducativos
 */
export const auditoriaActualizacionNivelesEducativosQueries = {
  /**
   * Obtiene el conteo total de niveles educativos actualizados
   */
  getCount: async (): Promise<number> => {
    const query = loadSQL('auditoria-actualizacion-niveles-educativos/get-count.sql');
    const result: QueryResult = await pool.query(query);
    return parseInt(result.rows[0].total, 10);
  },

  /**
   * Obtiene todas las actualizaciones de niveles educativos con filtros opcionales
   */
  getAll: async (filters?: {
    ...
    const query = loadSQL('auditoria-actualizacion-niveles-educativos/get-all.sql');
    const result: QueryResult = await pool.query(query, [
