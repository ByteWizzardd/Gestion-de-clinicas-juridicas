...
/**
 * Queries para la entidad AuditoriaActualizacionMaterias
 */
export const auditoriaActualizacionMateriasQueries = {
  /**
   * Obtiene el conteo total de materias actualizadas
   */
  getCount: async (): Promise<number> => {
    const query = loadSQL('auditoria-actualizacion-materias/get-count.sql');
    const result: QueryResult = await pool.query(query);
    return parseInt(result.rows[0].total, 10);
  },

  /**
   * Obtiene todas las actualizaciones de materias con filtros opcionales
   */
  getAll: async (filters?: {
    ...
    const query = loadSQL('auditoria-actualizacion-materias/get-all.sql');
    const result: QueryResult = await pool.query(query, [
