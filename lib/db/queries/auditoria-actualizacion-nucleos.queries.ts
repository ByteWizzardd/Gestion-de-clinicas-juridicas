...
/**
 * Queries para la entidad AuditoriaActualizacionNucleos
 */
export const auditoriaActualizacionNucleosQueries = {
  /**
   * Obtiene el conteo total de nucleos actualizados
   */
  getCount: async (): Promise<number> => {
    const query = loadSQL('auditoria-actualizacion-nucleos/get-count.sql');
    const result: QueryResult = await pool.query(query);
    return parseInt(result.rows[0].total, 10);
  },

  /**
   * Obtiene todas las actualizaciones de nucleos con filtros opcionales
   */
  getAll: async (filters?: {
    ...
    const query = loadSQL('auditoria-actualizacion-nucleos/get-all.sql');
    const result: QueryResult = await pool.query(query, [
