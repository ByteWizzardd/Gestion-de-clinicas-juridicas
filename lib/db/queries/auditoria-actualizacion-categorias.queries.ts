...
/**
 * Queries para la entidad AuditoriaActualizacionCategorias
 */
export const auditoriaActualizacionCategoriasQueries = {
  /**
   * Obtiene el conteo total de categorias actualizados
   */
  getCount: async (): Promise<number> => {
    const query = loadSQL('auditoria-actualizacion-categorias/get-count.sql');
    const result: QueryResult = await pool.query(query);
    return parseInt(result.rows[0].total, 10);
  },

  /**
   * Obtiene todas las actualizaciones de categorias con filtros opcionales
   */
  getAll: async (filters?: {
    ...
    const query = loadSQL('auditoria-actualizacion-categorias/get-all.sql');
    const result: QueryResult = await pool.query(query, [
